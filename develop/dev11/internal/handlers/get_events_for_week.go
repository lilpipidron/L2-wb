...
func GetEventsForWeek(storage *postgresql.Storage) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		query := storage.DB.Where("user_id = ? AND date >= ? AND date < ?::date + INTERVAL '1 week'", event.UserID, event.Date, event.Date)
