...
func GetEventsForMonth(storage *postgresql.Storage) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		query := storage.DB.Where("user_id = ? AND date >= ? AND date < ?::date + INTERVAL '1 month'", event.UserID, event.Date, event.Date)
